...
- **Worker (`services/cloudflare-online-verifier`):** `validateVerifyRequest` accepts an optional bounded non-negative integer `client_hardening` (reject out-of-range; default 0); the verifier **logs** it in the structured `verify.*` event and MAY record it for audit. It does **not** change the allow/deny decision.
