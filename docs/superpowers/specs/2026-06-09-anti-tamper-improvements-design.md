...
## Move 1 — server-anchored client hardening-posture telemetry (north star, do last)

**Design correction (2026-06-10):** the original framing — "thread the *tamper-detection verdict* into the online request" — does not work under the current code. Tamper policy is now binary (`DISABLED`/`ENFORCE`); under `ENFORCE` a detected signal returns `LICENSE_TAMPER_DETECTED` *before* the online call, and under `DISABLED` no signal is produced. So by the time the online request is built the detection verdict is always `0` — dead telemetry. The non-blocking `AUDIT` mode that made post-detection telemetry meaningful has been removed.

The meaningful, durable signal under the binary policy is the client's **hardening posture** (its configuration), which is always computable and non-fatal: the verifier learns which deployments run hardened, useful for rollout/audit and operator action, without becoming a tamper-policy engine.

- **C++ ABI (back-compatible extension):** extend `LccOnlineRequest` (version `1 → 2`, preserving the size/version-prefixed extension pattern) with a fixed-size `client_hardening` bitset (`uint32_t`): bit0 = tamper enforcement enabled, bit1 = host-integrity callback configured, bit2 = strict source-shadowing enabled, bit3 = online verification required. **No free-form text, no PII** — enumerated bits only. Populate it from `normalized_options` when building the online request in `acquire_license_with_runtime_checks` (it reflects configuration, not a runtime verdict, so it is well-defined whenever the online call runs).
- **Protocol:** the host transport callback serializes the request to the Worker; add an **optional** bounded integer `client_hardening` to the `/v1/verify` request body. Update `examples/online_callback` to forward it. Document the field.
- **Worker (`services/cloudflare-licensing-backend`):** `validateVerifyRequest` accepts an optional bounded non-negative integer `client_hardening` (reject out-of-range; default 0); the verifier **logs** it in the structured `verify.*` event and MAY record it for audit. It does **not** change the allow/deny decision.
- **Why telemetry, not enforcement (and why spoofing is fine):** a controlled client can lie about its posture. That is acceptable because the signal is *configuration evidence from honest clients*, not an authorization input. Treating it as deny-input would make the verifier trust an attacker-controlled bit and become a policy engine. Recording it preserves the durable value (operators see hardening coverage, can investigate) with zero new attack surface.
