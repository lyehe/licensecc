...
- Create: `services/cloudflare-licensing-backend/scripts/config-sign.mjs` — the offline signer.
...
- Node: `node services/cloudflare-licensing-backend/scripts/config-sign.mjs ...`
...
**Files:** Create `services/cloudflare-licensing-backend/scripts/config-sign.mjs`.

- [ ] **Step 1: Write the signer.** It mirrors `device-key.mjs` (arg parsing, validators, PEM/DER helpers) but builds the config canonical payload and signs RSA‑PKCS1‑SHA256 (the worker's algorithm). Create `services/cloudflare-licensing-backend/scripts/config-sign.mjs`:
...
```
node services/cloudflare-licensing-backend/scripts/config-sign.mjs 2>&1 | Select-Object -First 1
```
Expected: prints the `usage:` line and exits non-zero (no args). This confirms the script parses and the argument guard works.

- [ ] **Step 3: Commit.**
```bash
git add services/cloudflare-licensing-backend/scripts/config-sign.mjs
...
- [ ] **Step 1: Generate a dev key + fixtures.** Run these commands (PowerShell), which produce a dev RSA key, sign a fixed config, and capture the public key DER hex + the token. Use the existing `device-key.mjs` generate? No — that is ECDSA. Generate an RSA key with Node (the PEM boundary literals below are written `<PEM-BEGIN>` / `<PEM-END>` so the secret-hygiene scan does not flag this historical example; substitute the real `-----BEGIN/END PRIVATE KEY-----` lines when running):
```
