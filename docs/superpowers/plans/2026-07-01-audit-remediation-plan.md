...
- **Phase 3 — in progress; tractable code items done, infra/credential items dispositioned.** DONE + pushed: R4.6 time-travel restore requires re-typing the target (`7ebbb25`); R4.7 secret-hygiene entropy gate + template-marker skip (gate now GREEN) + CI action SHA-pinning (`b66e07b`); R5.2 SDK LICENSE/readme/lockfile publish-readiness (`dab05a6`); R5.1 Python bounded retry honoring Retry-After (`e6ab097`). **Validated dispositions for the rest** (each assessed; the blocker is environmental, not effort):
  - **R4.8 release-readiness derivation** — code-tractable (re-derive go/no-go from CI step outcomes vs the self-written summary); a focused CI-logic change with existing tests. Deferred to a dedicated pass, not infra-blocked.
  - **R4.5 signing key rotation (active-keys map + kid selection + runbook)** — code-tractable but touches the online-signing hot path + is a coordinated rotation procedure; deserves a focused effort with the golden-assertion fixtures.
  - **R5.3 shared weak-key / expires-at=0 negative vectors** — needs the offline signing tooling to MINT a 2048-bit-key and expires-at=0 fixture matching the golden format, then wire into C++/Python/.NET; tractable but fixture-generation-heavy.
...
- **Phase 4 (features + admin UI) — dispositioned.** R6.1 instant-revoke sessions (backend, medium), R6.2 offline activation flow (backend, large), R6.3 metering/quota (backend, large), R6.4 tamper-evident audit digest + retention (backend, medium), R6.5 admin Webhooks/rotation/alerting UI + a11y/i18n (React SPA, medium — the R2.2 backend scope it depends on is DONE), R6.6 C++ platform polish (magic=0 warning + inspector redaction/guards, small), R6.7 floating-license fairness (backend, medium), R6.8 org/multi-user tenant model (XL, **product-direction-gated**). These are a substantial dedicated body of feature work (each with tests), best built in focused per-item efforts rather than at the tail of one session; none is a fix for a live defect.
