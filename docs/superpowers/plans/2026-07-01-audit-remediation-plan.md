...
**Post-ship adversarial multi-agent review remediation (`349f40d`, `7e7fc98`, `b3edd5e`, `c062461` + the activation/device commits `a0fee63`/`3b0b4ca`).** An independent multi-dimension review of this session's work (with per-finding skeptical verification + a completeness critic) found real gaps the green tests missed — all now FIXED: **(HIGH, activation)** an argument/command-injection in `lcc-inspector --decode-activation-request` (untrusted request fields interpolated into a copy-paste `lccgen` command on the key-holding host) → strict per-field charsets at parse+build (`a0fee63`). **(HIGH, metering)** `meter_quota`/`meter_period_sec` had NO write path (dead enforcement) → added to `CAPACITY_COLUMNS` (+ `EntitlementCapacity` type) so the quota is configurable via order-ingest/admin; end-to-end test. **(MED)** no `usage_meters` retention → `scheduled()` sweep; SQLite-only `INSERT OR IGNORE` (breaks under the pg adapter the schema advertises) → portable `ON CONFLICT DO NOTHING`. **(LOW)** `units` unbounded (int64/2^53 corruption) → `Number.isSafeInteger` + cap; admin webhook client validators missed the server's `\n/\r/\0` rejection (confusing round-trip 400) → mirrored; device revoke surfaced as an indistinguishable generic `update` → `detail` added to the admin events feed + UI + webhook payload. Device-revoke backend reviewed **CLEAN** (the EXISTS-guard hardening `3b0b4ca` independently confirmed fail-closed; 6 refuted findings). **Remaining LOW/by-design dispositions (documented, not code-changed):** (a) `/v1/meter` reuses the `report` account-token op for a write — a least-privilege smell, but per-customer isolated + self-directed (a leaked read token can only pollute its OWN entitlement's counter), and a distinct `meter` op would fan out across the portal/CLI minters; deliberate trade-off. (b) `/v1/meter` is token-gated not D1-rate-limited — **consistent with every account-authed sibling** (seats/lease); only the unauthenticated `/v1/verify` needs the limiter. (c) break-glass `/v1/emergency/v1/meter` runs isolation-off — consistent with all `/v1/emergency/*` routes (by design, audited). (d) no read-only accessor for meter consumption (observable only by incrementing) — a genuine follow-on feature-add, not a defect.

**R6.3 metering — COMPLETED end-to-end (`e7395e9`, `63fabcc`).** The two follow-ons above that were real feature gaps are now shipped: **(1) quota is configurable** (`e7395e9`) — migration 0024 adds `meter_quota`/`meter_period_sec` to `entitlement_policies`; `stampFromPolicy` carries them into a stamped entitlement (they were already in `CAPACITY_COLUMNS`); admin policy create/validate/patch + editor UI fields + OpenAPI. So an operator defines a "metered" policy in the console and every stamped entitlement inherits + enforces the quota (proven by a policy-stamp→entitlement test). **(2) consumption is observable** (`63fabcc`) — new `GET /api/admin/entitlements/{id}/meter` (reader+admin, non-mutating) reports quota + the current period's `units_consumed` via a separate projection (shared `findEntitlement`/`ENTITLEMENT_COLUMNS` untouched), surfaced by a "Meter" toggle per entitlement row. R6.3 now spans storage → enforcement → **configurable quota → observable consumption**, all green (backend test:sql 109, admin npm test 53 + test:ui 43 + test:sql 60, both parity gates).
...
### R6.5 [MED gap] Admin console: webhooks/rotation/alerting UI + a11y/i18n — Webhooks slice DONE (`6370525`)
...
**Webhooks-management slice DONE.** New **Webhooks tab** in the admin SPA (`services/cloudflare-license-admin/`): endpoint editor (url/event-types/description + the R2.2 project/customer scope), a scoped endpoints table (disable/reenable + per-endpoint "Deliveries"), and a recent-deliveries pane (status filter + per-delivery **Redrive** for failed rows) — wired to the existing R2.2 worker CRUD (`GET/POST/PATCH /api/admin/webhooks*`, `.../disable|reenable`, `.../deliveries`, `.../deliveries/{id}/redrive`). All path/normalize/confirm logic lives in `operatorWorkflow.ts` (the node-testable layer) and **re-implements the Worker's webhook validators** (https-only URL, whitespace-free event-type tokens, comma-free bounded scopes) so the client rejects what the server would; `test/admin-ui-workflow.test.mjs` gains 5 cases (paths/encoding, action rules, form normalize+rejections, confirm copy) → **40 workflow tests green**. Fixed the OpenAPI drift the audit flagged: the three webhook schemas now declare `scope_project`/`scope_customer_id`. **a11y on the new view:** `scope="col"` headers, screen-reader `<caption>`s (new `.srOnly` utility), `aria-label`ed filter selects + a labelled deliveries region. Verified: tsc-worker + vite-ui build, `test:ui` 40, `npm test` 53 (worker + OpenAPI cross-check), lint. **Remaining follow-ons (honest scope):** (1) ~~device-revoke UI is BLOCKED~~ **DONE (closes R6.1)** — added the admin-worker endpoints `GET /api/admin/entitlements/{id}/devices` + `POST .../devices/{deviceKeyId}/(revoke|disable|reenable)` backed by a new shared `transitionEntitlementDevice`/`listEntitlementDevices` in the licensing-backend mutation core (atomic device status UPDATE + `revocation_seq` bump + constraint-safe `event_type='update'` audit row, mirroring the CLI), plus a per-entitlement **Devices pane** in the Entitlements tab (revoke/disable/reenable, a11y table). OpenAPI paths+schemas (`EntitlementDevice`/`DevicesListData`) + bidirectional cross-check (4 routes) + `test/sql/device-transition.test.mjs` (6 cases: transition+seq-bump+audit, terminal revoke, no-op, not-found, list) + 3 UI workflow tests. Verified: backend test:sql 106, admin build + test:ui 43 + npm test 53 + lint. (2) **key-rotation + alerting views** (no backend surface yet). (3) **full i18n** (all copy is still hardcoded English; a real string-catalog scaffold is a dedicated effort — not stubbed half-way). (4) **Playwright e2e walkthrough** for the tabs (browser QA deferred per environment; the existing e2e is unaffected — it targets buttons by name and never navigates to the new views).
