...
> **STATUS — COMPLETE (2026-06-25).** All of T1–T9 are landed on `feature/operations-back-office`
> (`09a3122`→`71bd999`). Slices 0–4 shipped design→attack→harden with adversarially-verified, real-SQLite
> test matrices. Final green state across all four services: backend **210** unit + **66** real-SQLite + lint
> + schema-parity; admin **43** + **10** SQL + **10** UI + e2e + lint; customer-portal **50** + **7** UI + e2e +
> lint; d1-backup **35** + lint. Nothing pushed (local `lyehe`-only policy; awaiting the user's push call).
...
- [x] **T2 (P1)** — foundation — DONE (commits `b81e838` mechanism + `57e038d` bulk move). Extracted the entitlement-mutation core into the shared `entitlement_mutation.mjs` both Workers import via a **package dependency** (backend `exports` + admin `file:` dep + bare specifier — the `.mjs` re-export shim was a dead end, doesn't compile into the admin `dist-worker`). `createEntitlement` is byte-identical (its `ON CONFLICT` writes 0 seat cols → no clobber); seat/device writes live in a separate allowlist-driven `setEntitlementCapacity()` for Slice 1's order-ingest. Types: Option A — the `.d.ts` is the single canonical home, api.ts re-exports. Verified: admin build clean + 43/43, backend 112/112 (6 new), parity ok, CI gap (admin workflow now watches `src/entitlements/**`) + case-insensitive email folded from /review.
...
- [x] **T4 (P1)** — fulfillment — DONE (Slice 1, commits `252cc2e`→`9e5e3c8`). `POST /v1/orders`: HMAC verify (raw-bytes, key_id rotation, skew, nonce) + atomic ACCEPT (guarded cursor + claim) + floor-guarded APPLY + in-txn processed-mark; cancel→disabled; D6 fingerprint + ownership invariant; crash-redrive. Verified on REAL SQLite — 18-case adversarial matrix (crash-redrive-no-regress, concurrent N/N+1 floor-no-op, fingerprint_owned 409, seat reclaim, valid_until clamp, missing/revoked, carry-forward) + signer round-trip. Backend 179/179, test:sql 40/40, admin 43/43, parity ok. Blueprint: `docs/superpowers/plans/2026-06-24-slice1-order-ingest-blueprint.md`. **PG runtime apply-path follow-up DONE** (commit `10d2659`): `supabase-postgres/order-apply-pg.mjs` ports the apply SQL + `runApplyTransaction` (exactly-once floor/ownership/processed-mark preserved; `json_object`→`json_build_object::text`, `rowid`→`ctid`, `max(0,..)`→`GREATEST` translated), hermetic `test:pg` 17/17 + a `DATABASE_URL`-gated real-PG smoke, adversarially reviewed (0 confirmed findings). Scope = SQL + transaction runner; the live PG `POST /v1/orders` HTTP runtime stays deferred.
- [x] **T5 (P1)** — identity — DONE (Slice 2, commits `238a7f5`→`5ce917f`). `account_tokens` (keyed HMAC + `pepper_key_id`, scopes, rotation, emergency revoke) + `customer_id` bound via an `EXISTS`-on-the-mutating-statement on the 6 scoped paths + staged off→soft→required cutover (shadow-eval) + operator CLI. Round-2-hardened (F1 idempotency-by-customer, F3 status-in-guard, F6 soft-gate). Verified on REAL SQLite — 17-case isolation matrix (A-on-B deny no-oracle, F1 replay-no-leak, F3 revoked-mints-nothing, F6, foreign-seats-dont-count, emergency route). Backend 208/208, admin 43/43, parity+lint ok. Blueprint: `docs/superpowers/plans/2026-06-24-slice2-account-token-blueprint.md`.
- [x] **T6 (P1)** — portal — DONE (Slice 3, commits `4b141cd`→`22825f1`). New `services/cloudflare-customer-portal/` Worker + React SPA (same D1): email-OTP/magic-link login, my-entitlements/devices/usage, download-.lic proxied via the lease Worker (portal never signs), per-action ephemeral account_token mint (never persisted/sent to browser), server-side tuple resolution (no existence oracle). Verified: 21-cell IDOR matrix on real SQLite (75 asserts), portal 50/50 + UI 7/7 + e2e leak-guard, parity+lint ok. Blueprint: `docs/superpowers/plans/2026-06-24-slice3-customer-portal-blueprint.md`.
- [x] **T7 (P2)** — enforcement — DONE (commit `71bd999`). (1) Seat token offline deadline clamped to `valid_until` (checkout live+borrow, heartbeat) — bounds offline borrow to the entitlement window. (2) Heartbeat UPDATE folds `status='active'`+validity (all modes) — revoked/disabled/expired seats stop refreshing and lapse within one grace; closes the one-heartbeat TOCTOU. (3) Downgrade reclaim (`SEAT_OVERCAP_RECLAIM_SQL`, hot path + cron) — over-ceiling live seats reclaimed (latest-alive kept, borrowed never deleted) so a pool cut takes effect within one sweep+grace. Leases already SLA-correct (valid_to clamp + revoke-denies-renewal). Verified: backend 210/210 (+2 clamp), test:sql 66/66 (+9 real-SQLite SLA suite), lint ok.
- [x] **T8 (P2)** — console — DONE (Slice 4, commits `a124ef1`→`f380546`). Migration 0017 (`customer_events`) + admin Worker read surface (customers list/detail, licenses, fulfillment monitor, report dashboard — all reader+admin) + the admin-only customer kill-switch (`POST /customers/:id/disable|reenable`, atomic guarded-UPDATE+audit batch, severs account-token auth via the `customers.status='active'` join) + React tabs. Verified: real-SQLite console suite (8 cases: no-token_hmac-leak, fulfillment stale, report aggregates, kill-switch atomicity+gates+idempotency, reader-RBAC-blocks-the-write matrix), admin 43/43 + test:sql 10/10 + test:ui 10/10 + e2e, parity ok. Also repaired the pre-existing stale `audit-json-object` source pointer.
- [x] **T9 (P2)** — ops — DONE (commit `9809b07`). Restore-drill `REQUIRED_TABLES` extended to the durable back-office tables (count-compared); new `PRESENCE_ONLY_TABLES` (high-churn/ephemeral — presence-asserted, not count-compared) + `SENSITIVE_TABLES` (secret/PII — drill only ever runs COUNT(*)/sqlite_master, never content; summary lists names only). Verified: all 21 schema tables resolve (no typos), d1-backup 35/35, content-free-SQL test pins the guarantee.
