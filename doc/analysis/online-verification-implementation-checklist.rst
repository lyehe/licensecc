...
* Document rollout, audit mode, enforcement mode, cache behavior, and failure
  semantics.
...
* Add online result codes after existing values, for example:
  ...
  * ``LICENSE_ONLINE_CACHE_EXPIRED``

* Add online policy enum:

  * ``LCC_ONLINE_DISABLED``: no callback, no assertion verification;
  * ``LCC_ONLINE_AUDIT``: record online failures as warnings and keep a locally
    valid license accepted;
  * ``LCC_ONLINE_REQUIRE``: require a fresh valid server assertion;
  * ``LCC_ONLINE_REQUIRE_WITH_CACHE``: require fresh assertion or valid offline
    cache entry.
...
* Update ``lcc_init_license_check_options()``:
  ...
  * tamper defaults remain audit mode;
  * online defaults to disabled;
...
* Bind the assertion to the current request:
  ...
  * cache-until is not later than the documented maximum.
...
* Add policy evaluation:
  ...
  * audit: call callback when configured and record warning on failure;
  * require: deny unless callback returns a valid fresh assertion;
  * require-with-cache: deny unless callback returns valid fresh assertion or
...
* Add deterministic result-code mapping:
  ...
  * expired required cache returns ``LICENSE_ONLINE_CACHE_EXPIRED``;
...
* disabled policy does not invoke callback;
* audit policy invokes callback and preserves ``LICENSE_OK`` on transport
  failure;
...
* require-with-cache accepts valid cache when callback transport fails;
* require-with-cache rejects expired cache;
...
* Preserve anti-tamper behavior:
  ...
  * if anti-tamper audit reports warnings, online verification may still run.
...
* Export online events through ``EventRegistry`` with explicit severity:
  ...
  * audit policy warnings use ``SVRT_WARN``;
  * enforcement failures use ``SVRT_ERROR``;
...
Gate 5: offline cache and revocation semantics
==============================================

**Purpose**


Allow low-volume users to tolerate brief Cloudflare or network outages without
turning online verification into a permanent bypass.

**Entry criteria**


* Gate 4 is complete.
* The cache threat model is documented.

**Implementation checklist**


* Make cache optional and policy-driven.
* Do not write cache files from core C++ unless a cache callback or explicit
  host-owned storage API is added.
* Prefer host-owned cache storage:

  * core returns the verified assertion and cache-until value;
  * host stores it in an application-specific location;
  * host supplies cached assertion through the same callback or a separate
    callback.

* Bind cache entries to:

  * project;
  * feature;
  * license fingerprint;
  * device hash when used;
  * revocation sequence;
  * cache-until.

* Enforce maximum cache TTL in core.
* Treat cache expiration as fail-closed in require-with-cache mode.
* Do not allow cache to override an explicit server denial returned during a
  fresh online check.
* Define revocation sequence behavior:

  * server includes monotonically increasing sequence per license or account;
  * client accepts cached assertion only until cache-until;
...
* valid cached assertion accepted when transport fails;
* expired cached assertion rejected;
* cached assertion for different feature rejected;
* cached assertion for different license fingerprint rejected;
* cached assertion with too-long cache-until rejected;
* fresh denial overrides cache;
* disabled cache policy ignores cached assertion.

**Exit criteria**


* Cache behavior is deterministic and bounded.
* No implicit cache file path is introduced in core without explicit approval.
* Documentation makes clear that cache is resilience, not revocation certainty.
...
* Update security model:
  ...
  * audit mode is recommended first;
  * enforcement requires false-positive testing;
  * cache is bounded resilience, not real-time revocation.
...
* Rollback plan exists:
  ...
  * hosts can set ``LCC_ONLINE_DISABLED``;
  * audit mode can be used during rollout;
...
* Audit mode never denies an otherwise valid local license.
* Require mode denies missing, failed, invalid, expired, or denied assertions.
* Require-with-cache accepts only bounded, matching, unexpired cache assertions.
