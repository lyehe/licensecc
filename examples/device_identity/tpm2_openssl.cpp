...
int main(int argc, char** argv) {
	if (argc < 4) {
		std::cerr << "usage: licensecc_tpm2_openssl <application-id> <project> <storage-directory> "
					 "[--machine] [--create]\n";
		return 2;
	}
	...
	if (!set_field(options.application_id, argv[1]) || !set_field(options.project, argv[2]) ||
		!set_field(options.storage_directory, argv[3])) {
		std::cerr << "application id, project, or storage directory exceeds the public ABI bound\n";
		return 2;
	}
	...
	if (metadata_result != LCC_DEVICE_OK || exported != LCC_DEVICE_OK ||
		metadata.backend != LCC_DEVICE_BACKEND_TPM2_OPENSSL || std::string(metadata.provider) != "tpm2-openssl") {
		std::cerr << "TPM2 device identity inspection failed\n";
		return 1;
	}
