
#include "../../base/file_utils.hpp"
#include "../../base/string_utils.h"
#include "../../base/logger.h"
#include "../dmi_info.hpp"

namespace license {
namespace os {

DmiInfo::DmiInfo() {
	try {
		m_bios_vendor = toupper_copy(trim_copy(get_file_contents("/sys/class/dmi/id/sys_vendor", 256)));
	} catch (const std::exception& e) {
		m_bios_vendor = "";
		LOG_DEBUG("Can not read sys_vendor %s", e.what());
	}
	try {
		// trim_copy already strips any trailing newline; indexing the raw string
		// underflowed when the dmi file was empty or shorter than expected.
		m_bios_description = toupper_copy(trim_copy(get_file_contents("/sys/class/dmi/id/modalias", 256)));
	} catch (const std::exception& e) {
		m_bios_description = "";
		LOG_DEBUG("Can not read bios_description %s", e.what());
	}
	try {
		m_sys_vendor = trim_copy(get_file_contents("/sys/class/dmi/id/sys_vendor", 256));
	} catch (const std::exception& e) {
		m_sys_vendor = "";
		LOG_DEBUG("Can not read sys_vendor %s", e.what());
	}
}

}  // namespace os
}  // namespace license
