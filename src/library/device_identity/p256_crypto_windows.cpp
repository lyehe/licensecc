...
bool import_public_key(BCRYPT_ALG_HANDLE algorithm, const P256Spki& spki, KeyHandle& key) {
	...
	return NT_SUCCESS(BCryptImportKeyPair(algorithm, nullptr, BCRYPT_ECCPUBLIC_BLOB, &key.value, blob.value.data(),
										  static_cast<ULONG>(blob.value.size()), 0));
}
...
bool sha256(const std::uint8_t* data, std::size_t size, P256Digest& out) noexcept {
	try {
		...
		DWORD hash_size = 0U;
		DWORD written = 0U;
		if (!NT_SUCCESS(BCryptGetProperty(algorithm.value, BCRYPT_OBJECT_LENGTH, reinterpret_cast<PUCHAR>(&object_size),
		...
		HashHandle hash;
		if (!NT_SUCCESS(
				BCryptCreateHash(algorithm.value, &hash.value, object.value.data(), object_size, nullptr, 0U, 0))) {
			return false;
		}
		if (size != 0U &&
			!NT_SUCCESS(BCryptHashData(hash.value, const_cast<PUCHAR>(data), static_cast<ULONG>(size), 0))) {
			return false;
		}
		SensitiveArray<32> candidate;
		if (!NT_SUCCESS(
				BCryptFinishHash(hash.value, candidate.value.data(), static_cast<ULONG>(candidate.value.size()), 0))) {
			return false;
		}
		out = candidate.value;
		return true;
...
bool platform_validate_p256_spki(const P256Spki& spki) noexcept {
	try {
		AlgorithmHandle algorithm;
		if (!NT_SUCCESS(BCryptOpenAlgorithmProvider(&algorithm.value, BCRYPT_ECDSA_P256_ALGORITHM, nullptr, 0))) {
			return false;
		}
		KeyHandle key;
...
bool platform_verify_p256_p1363(const P256Spki& spki, const P256Digest& digest,
	...
	try {
		...
		return NT_SUCCESS(BCryptVerifySignature(key.value, nullptr, const_cast<PUCHAR>(digest.data()),
												static_cast<ULONG>(digest.size()), const_cast<PUCHAR>(signature.data()),
