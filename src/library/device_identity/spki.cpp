...
bool read_der_integer(const std::uint8_t* der, std::size_t size, std::size_t& offset, const std::uint8_t*& magnitude,
...
bool decode_canonical_base64(const std::string& encoded, std::vector<std::uint8_t>& out) noexcept {
	try {
		...
		for (std::size_t offset = 0U; offset < encoded.size(); offset += 4U) {
			...
			const int c = encoded[offset + 2U] == '='
							  ? -2
							  : decode_base64_character(static_cast<unsigned char>(encoded[offset + 2U]));
			const int d = encoded[offset + 3U] == '='
							  ? -2
							  : decode_base64_character(static_cast<unsigned char>(encoded[offset + 3U]));
			if (a < 0 || b < 0 || c == -1 || d == -1 || (!last && (c == -2 || d == -2)) || (c == -2 && d != -2)) {
				return false;
			}
			const std::uint32_t word = (static_cast<std::uint32_t>(a) << 18U) | (static_cast<std::uint32_t>(b) << 12U) |
