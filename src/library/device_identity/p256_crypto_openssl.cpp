...
bool sha256(const std::uint8_t* data, std::size_t size, P256Digest& out) noexcept {
	...
	if (EVP_Digest(size == 0U ? &empty : data, size, candidate.value.data(), &written, EVP_sha256(), nullptr) != 1 ||
...
bool platform_verify_p256_p1363(const P256Spki& spki, const P256Digest& digest,
	...
	try {
		...
		return EVP_PKEY_verify(context.get(), der.value.data(), der.value.size(), digest.data(), digest.size()) == 1;
	} catch (...) {
		return false;
	}
}
