...
constexpr ProviderContract kWindowsTpmProviderContract = {
	LCC_DEVICE_BACKEND_WINDOWS_TPM, LCC_DEVICE_ASSURANCE_REPORTED_HARDWARE, "windows-platform-ksp", kP256Algorithm};
constexpr ProviderContract kTpm2OpenSslProviderContract = {
	LCC_DEVICE_BACKEND_TPM2_OPENSSL, LCC_DEVICE_ASSURANCE_REPORTED_HARDWARE, "tpm2-openssl", kP256Algorithm};
constexpr ProviderContract kSoftwareTestProviderContract = {
	LCC_DEVICE_BACKEND_SOFTWARE_TEST, LCC_DEVICE_ASSURANCE_SOFTWARE, "software-test", kP256Algorithm};
...
inline bool provider_metadata_matches_contract(const ProviderMetadata& metadata, const ProviderOpenRequest& request) {
...
public:
	...
	virtual LCC_DEVICE_RESULT delete_with_expected_id(const ProviderOpenRequest& request,
													  const std::string& expected_device_key_id) noexcept = 0;
};
