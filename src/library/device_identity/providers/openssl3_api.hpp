...
public:
	...
	virtual EVP_PKEY_CTX* pkey_ctx_new_from_name(OSSL_LIB_CTX* libctx, const char* name,
												 const char* properties) noexcept = 0;
	virtual EVP_PKEY_CTX* pkey_ctx_new_from_pkey(OSSL_LIB_CTX* libctx, EVP_PKEY* key,
												 const char* properties) noexcept = 0;
	...
	virtual int pkey_sign(EVP_PKEY_CTX* context, unsigned char* signature, std::size_t* signature_size,
						  const unsigned char* digest, std::size_t digest_size) noexcept = 0;
	virtual int pkey_verify(EVP_PKEY_CTX* context, const unsigned char* signature, std::size_t signature_size,
							const unsigned char* digest, std::size_t digest_size) noexcept = 0;
	virtual EVP_MD* md_fetch(OSSL_LIB_CTX* libctx, const char* name, const char* properties) noexcept = 0;
	virtual void md_free(EVP_MD* digest) noexcept = 0;
	virtual EVP_PKEY* d2i_public_key(OSSL_LIB_CTX* libctx, const unsigned char* data, std::size_t size) noexcept = 0;
	virtual int digest(EVP_MD* digest, const unsigned char* data, std::size_t size, unsigned char* output,
	...
	virtual int pkey_get_utf8_string_param(const EVP_PKEY* key, const char* name, char* value, std::size_t value_size,
										   std::size_t* written) noexcept = 0;

	virtual OSSL_ENCODER_CTX* encoder_new_for_pkey(const EVP_PKEY* key, int selection, const char* output_type,
												   const char* output_structure, const char* properties) noexcept = 0;
	virtual int encoder_to_data(OSSL_ENCODER_CTX* context, unsigned char** data, std::size_t* data_size) noexcept = 0;
	...
	virtual OSSL_STORE_CTX* store_open_ex(const char* uri, OSSL_LIB_CTX* libctx, const char* properties,
										  const UI_METHOD* ui_method, void* ui_data) noexcept = 0;
	...
	virtual int rand_priv_bytes_ex(OSSL_LIB_CTX* libctx, unsigned char* data, std::size_t size,
