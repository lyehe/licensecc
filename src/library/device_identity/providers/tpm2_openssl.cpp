...
constexpr std::array<std::uint8_t, 27> kP256SpkiPrefix = {
	{0x30U, 0x59U, 0x30U, 0x13U, 0x06U, 0x07U, 0x2aU, 0x86U, 0x48U, 0xceU, 0x3dU, 0x02U, 0x01U, 0x06U,
	 0x08U, 0x2aU, 0x86U, 0x48U, 0xceU, 0x3dU, 0x03U, 0x01U, 0x07U, 0x03U, 0x42U, 0x00U, 0x04U}};
...
public:
	...
	EVP_PKEY_CTX* pkey_ctx_new_from_name(OSSL_LIB_CTX* libctx, const char* name,
										 const char* properties) noexcept override {
		return EVP_PKEY_CTX_new_from_name(libctx, name, properties);
	}
	EVP_PKEY_CTX* pkey_ctx_new_from_pkey(OSSL_LIB_CTX* libctx, EVP_PKEY* key,
										 const char* properties) noexcept override {
	...
	int pkey_sign(EVP_PKEY_CTX* context, unsigned char* signature, std::size_t* signature_size,
				  const unsigned char* digest, std::size_t digest_size) noexcept override {
		return EVP_PKEY_sign(context, signature, signature_size, digest, digest_size);
	}
	int pkey_verify(EVP_PKEY_CTX* context, const unsigned char* signature, std::size_t signature_size,
					const unsigned char* digest, std::size_t digest_size) noexcept override {
	...
	EVP_PKEY* d2i_public_key(OSSL_LIB_CTX* libctx, const unsigned char* data, std::size_t size) noexcept override {
		if (data == nullptr || size > static_cast<std::size_t>(LONG_MAX)) {
			return nullptr;
		}
		const unsigned char* cursor = data;
		EVP_PKEY* key = d2i_PUBKEY_ex(nullptr, &cursor, static_cast<long>(size), libctx, "provider=default");
	...
	int digest(EVP_MD* digest, const unsigned char* data, std::size_t size, unsigned char* output,
			   std::size_t* output_size) noexcept override {
		unsigned int written = 0U;
		const unsigned char empty = 0U;
		const int result = EVP_Digest(size == 0U ? &empty : data, size, output, &written, digest, nullptr);
	...
	int pkey_get_utf8_string_param(const EVP_PKEY* key, const char* name, char* value, std::size_t value_size,
								   std::size_t* written) noexcept override {
		return EVP_PKEY_get_utf8_string_param(key, name, value, value_size, written);
	}

	OSSL_ENCODER_CTX* encoder_new_for_pkey(const EVP_PKEY* key, int selection, const char* output_type,
	...
	int encoder_to_data(OSSL_ENCODER_CTX* context, unsigned char** data, std::size_t* data_size) noexcept override {
	...
	OSSL_STORE_CTX* store_open_ex(const char* uri, OSSL_LIB_CTX* libctx, const char* properties,
								  const UI_METHOD* ui_method, void* ui_data) noexcept override {
	...
	int rand_priv_bytes_ex(OSSL_LIB_CTX* libctx, unsigned char* data, std::size_t size,
...
public:
	...
	int linkat(int old_directory, const char* old_path, int new_directory, const char* new_path,
...
private:
	...
	static void copy_text(const char* source, std::array<char, Size>& destination, std::size_t& destination_size,
	...
	void restore_caller_queue() noexcept {
		for (std::size_t index = 0U; index < preserved_count_; ++index) {
			...
			ERR_new();
			ERR_set_debug(record.has_file ? record.file.data() : nullptr, record.line,
						  record.has_function ? record.function.data() : nullptr);
			ERR_set_error(ERR_GET_LIB(record.code), ERR_GET_RFLAGS(record.code) | ERR_GET_REASON(record.code), nullptr);
...
LCC_DEVICE_RESULT map_provider_error_text(int saved_errno, bool loading_reference, const std::string& text) {
	...
	if (text.find("cannot connect") != std::string::npos || text.find("cannot get capability") != std::string::npos ||
	...
	if (text.find("exhaust") != std::string::npos || text.find("resource manager") != std::string::npos ||
		...
		text.find("insufficient resources") != std::string::npos || text.find("retry") != std::string::npos ||
		text.find("try again") != std::string::npos || text.find("timeout") != std::string::npos ||
		text.find("lock") != std::string::npos || text.find("busy") != std::string::npos ||
		text.find("too many") != std::string::npos || text.find("limit") != std::string::npos) {
		return LCC_DEVICE_BUSY;
	}
	if (loading_reference &&
		(text.find("input corrupted") != std::string::npos || text.find("wrong data length") != std::string::npos ||
...
LCC_DEVICE_RESULT open_storage_directory(const std::shared_ptr<PosixStorageApi>& api, const std::string& path,
										 DirectoryHandle& out) {
	if (!api) {
		return LCC_DEVICE_INTERNAL_ERROR;
	}
	...
	for (std::size_t index = 0U; index < components.size(); ++index) {
		...
		const int next = api->openat(current, components[index].c_str(),
									 selected ? kSelectedDirectoryOpenFlags : kDirectoryOpenFlags, 0U);
		if (next < 0) {
			const int saved_errno = errno;
			(void)api->close(current);
			return saved_errno == EACCES || saved_errno == EPERM || saved_errno == ELOOP ? LCC_DEVICE_ACCESS_DENIED
				   : saved_errno == ENOENT												 ? LCC_DEVICE_KEY_NOT_FOUND
																						 : LCC_DEVICE_IO_ERROR;
		}
		struct stat status {};
...
public:
	...
	LCC_DEVICE_RESULT acquire(int directory, const std::string& name) noexcept {
		...
		descriptor_ =
			api_->openat(directory, name.c_str(), O_CREAT | O_RDWR | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK, 0600U);
		if (descriptor_ < 0) {
			return errno == EACCES || errno == EPERM ? LCC_DEVICE_ACCESS_DENIED : LCC_DEVICE_IO_ERROR;
		}
		struct stat status {};
		...
		for (;;) {
			...
			const struct timespec pause {
				pause_ns / 1000000000LL, pause_ns % 1000000000LL
			};
...
bool valid_tss2_private_pem(const unsigned char* data, std::size_t size) noexcept {
	...
	if (data == nullptr || size <= kBeginSize + kEndSize || std::memcmp(data, kBegin, kBeginSize) != 0) {
		return false;
	}
	...
	if (content_end < kBeginSize + kEndSize || std::memcmp(data + content_end - kEndSize, kEnd, kEndSize) != 0) {
		return false;
	}
...
public:
	...
	LCC_DEVICE_RESULT create(const ProviderOpenRequest& request) noexcept override {
		...
		try {
			...
			result = write_temporary(directory.descriptor, request.device_namespace.linux_filename, pem.value,
									 temporary_name, temp_identity);
			if (result != LCC_DEVICE_OK) {
				return result;
			}
			...
			const auto rollback_transaction = [&]() noexcept {
				...
				if (published) {
					try {
						...
						const LCC_DEVICE_RESULT final_result = remove_owned(
							directory.descriptor, request.device_namespace.linux_filename, temp_identity, removed);
				...
				try {
					const LCC_DEVICE_RESULT temporary_result =
			...
			try {
				result =
					publish_temporary(directory.descriptor, temporary_name, request.device_namespace.linux_filename,
									  published, cleanup_attempted, temp_identity);
				if (result != LCC_DEVICE_OK) {
	...
	LCC_DEVICE_RESULT delete_with_expected_id(const ProviderOpenRequest& request,
		...
		try {
			...
			bool removed = false;
			const LCC_DEVICE_RESULT removed_result =
				remove_owned(directory.descriptor, request.device_namespace.linux_filename, loaded.identity, removed);
...
private:
	...
	public:
		PkeyHandle(std::shared_ptr<OpenSsl3Api> api, EVP_PKEY* key) : api_(std::move(api)), key_(key) {}
		~PkeyHandle() {
	...
	public:
		MessageDigest(std::shared_ptr<OpenSsl3Api> api, EVP_MD* digest) : api_(std::move(api)), digest_(digest) {}
		~MessageDigest() {
	...
	LCC_DEVICE_RESULT validate_request(const ProviderOpenRequest& request) const {
		if (request.backend != LCC_DEVICE_BACKEND_TPM2_OPENSSL ||
			...
			request.device_namespace.linux_filename != "licensecc-v1-" + request.device_namespace.hash + ".tss2.pem" ||
	...
	LCC_DEVICE_RESULT ensure_context() {
		...
		if (default_provider_ == nullptr) {
			...
			const bool provider_module_missing =
				(error_text.find("not found") != std::string::npos &&
				 (error_text.find("provider") != std::string::npos || error_text.find("module") != std::string::npos ||
			...
			unload_context();
			return provider_module_missing && !allocation_failure ? LCC_DEVICE_PROVIDER_UNAVAILABLE : provider_error;
		}
		clear_provider_error_queue();
		...
		if (tpm2_provider_ == nullptr) {
			...
			const bool allocation_failure = error_text.find("memory allocation failure") != std::string::npos ||
											error_text.find("allocation failure") != std::string::npos;
			unload_context();
			return provider_error == LCC_DEVICE_HARDWARE_UNAVAILABLE || provider_error == LCC_DEVICE_ACCESS_DENIED ||
						   provider_error == LCC_DEVICE_BUSY
					   ? provider_error
				   : provider_module_missing && !allocation_failure ? LCC_DEVICE_PROVIDER_UNAVAILABLE
																	: provider_error;
		}
		return LCC_DEVICE_OK;
	}
	...
	LCC_DEVICE_RESULT generate_key(EVP_PKEY** out) {
		...
		*out = nullptr;
		...
		const OSSL_PARAM params[] = {OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, group, 0U),
	...
	LCC_DEVICE_RESULT validate_key(EVP_PKEY* key, P256Spki& out) {
		...
		if (openssl_->pkey_get_utf8_string_param(key, OSSL_PKEY_PARAM_GROUP_NAME, group.data(), group.size(),
												 &group_size) <= 0 ||
			std::string(group.data(), group_size) != "prime256v1") {
			return LCC_DEVICE_UNSUPPORTED_ALGORITHM;
		}
		clear_provider_error_queue();
		OSSL_ENCODER_CTX* raw_encoder =
			openssl_->encoder_new_for_pkey(key, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", nullptr);
		...
		clear_provider_error_queue();
		EVP_PKEY* decoded_public =
			prefix_ok ? openssl_->d2i_public_key(libctx_, encoded_buffer.get(), encoded_buffer.size()) : nullptr;
	...
	LCC_DEVICE_RESULT self_test(EVP_PKEY* key, const P256Spki& spki) {
		...
		clear_provider_error_queue();
		PkeyContext verify_context(openssl_,
								   openssl_->pkey_ctx_new_from_pkey(libctx_, public_key.get(), "provider=default"));
		...
		clear_provider_error_queue();
		const int verified =
			openssl_->pkey_verify(verify_context.get(), der.data(), der.size(), digest.data(), digest.size());
	...
	LCC_DEVICE_RESULT encode_private_reference(EVP_PKEY* key, std::vector<std::uint8_t>& out) {
		clear_provider_error_queue();
		OSSL_ENCODER_CTX* raw_encoder =
			openssl_->encoder_new_for_pkey(key, EVP_PKEY_KEYPAIR, "PEM", "PrivateKeyInfo", "provider=tpm2");
		if (raw_encoder == nullptr) {
			return map_provider_error(0, false);
		}
		unsigned char* encoded = nullptr;
		...
		EncodedBuffer encoded_buffer(openssl_);
		encoded_buffer.assign(encoded, encoded_size);
		if (encoded_ok != 1 || encoded == nullptr ||
			!valid_tss2_private_pem(encoded_buffer.get(), encoded_buffer.size())) {
			return LCC_DEVICE_KEY_CORRUPT;
		}
	...
	LCC_DEVICE_RESULT load_reference(int directory, const std::string& filename, LoadedReference& out) {
		...
		if (raw_descriptor < 0) {
			...
			return errno == EACCES || errno == EPERM ? LCC_DEVICE_ACCESS_DENIED
				   : errno == ELOOP					 ? LCC_DEVICE_KEY_CORRUPT
													 : LCC_DEVICE_IO_ERROR;
		}
		...
		errno = 0;
		clear_provider_error_queue();
		StoreContext store(openssl_,
						   openssl_->store_open_ex(uri.c_str(), libctx_, "?provider=tpm2", ui_method, nullptr));
		...
		if (close_result != 1) {
			return close_error.empty() ? LCC_DEVICE_INTERNAL_ERROR : map_provider_error_text(0, true, close_error);
		}
		if (duplicate || null_pkey || pkey_count != 1U || key.get() == nullptr) {
			return LCC_DEVICE_KEY_CORRUPT;
		}
		if (store_error != 0 && !terminal_store_probe) {
			return load_error.empty() ? map_provider_error(0, true) : map_provider_error_text(0, true, load_error);
		}
		P256Spki spki{};
	...
	LCC_DEVICE_RESULT write_temporary(int directory, const std::string& filename, const std::vector<std::uint8_t>& data,
									  std::string& out_name, FileIdentity& out_identity) {
		...
		const int descriptor =
			posix_->openat(directory, out_name.c_str(), O_CREAT | O_EXCL | O_WRONLY | O_CLOEXEC | O_NOFOLLOW, 0600U);
		if (descriptor < 0) {
			return errno == EEXIST					   ? LCC_DEVICE_BUSY
				   : errno == EACCES || errno == EPERM ? LCC_DEVICE_ACCESS_DENIED
													   : LCC_DEVICE_IO_ERROR;
		}
		struct stat status {};
		...
		if (stat_result != 0) {
			...
			return stat_errno == EACCES || stat_errno == EPERM ? LCC_DEVICE_ACCESS_DENIED : LCC_DEVICE_IO_ERROR;
		}
		if (!valid_reference_status(status)) {
			(void)posix_->close(descriptor);
	...
	LCC_DEVICE_RESULT cleanup_temporary(int directory, const std::string& name, const FileIdentity& expected_identity) {
		bool removed = false;
		...
		return result == LCC_DEVICE_OK && removed	? LCC_DEVICE_OK
			   : result == LCC_DEVICE_KEY_NOT_FOUND ? LCC_DEVICE_OK
													: result;
	}
	...
	static LCC_DEVICE_RESULT storage_errno_result(int saved_errno) noexcept {
		return saved_errno == EACCES || saved_errno == EPERM								 ? LCC_DEVICE_ACCESS_DENIED
			   : saved_errno == EAGAIN || saved_errno == EWOULDBLOCK || saved_errno == EBUSY ? LCC_DEVICE_BUSY
																							 : LCC_DEVICE_IO_ERROR;
	}
	...
	LCC_DEVICE_RESULT remove_exact(int directory, const std::string& filename, const FileIdentity& expected,
								   bool& removed, bool require_safe_mode = true) {
		removed = false;
		const int raw_descriptor = posix_->openat(directory, filename.c_str(), kReferenceOpenFlags, 0U);
		if (raw_descriptor < 0) {
		...
		if (!valid_removal_status(status, require_safe_mode)) {
			const LCC_DEVICE_RESULT result =
				S_ISREG(status.st_mode) && status.st_uid == ::geteuid() ? LCC_DEVICE_BUSY : LCC_DEVICE_ACCESS_DENIED;
			return result;
		}
		...
		if (posix_->linkat(AT_FDCWD, descriptor_path.c_str(), directory, quarantine.c_str(), AT_SYMLINK_FOLLOW) != 0) {
			const int saved_errno = errno;
			return saved_errno == EACCES || saved_errno == EPERM ? LCC_DEVICE_ACCESS_DENIED
				   : saved_errno == EEXIST						 ? LCC_DEVICE_BUSY
																 : LCC_DEVICE_IO_ERROR;
		}
		...
		if (posix_->renameat2_noreplace(directory, filename.c_str(), moved.c_str()) != 0) {
			const int saved_errno = errno;
			const bool rename_unsupported = saved_errno == ENOSYS || saved_errno == EINVAL || saved_errno == EOPNOTSUPP;
			const LCC_DEVICE_RESULT cleanup_result = cleanup_quarantine(directory, quarantine);
			if (cleanup_result != LCC_DEVICE_OK) {
		...
		if (!moved_matches) {
			...
			const bool restore_unsupported =
	...
	LCC_DEVICE_RESULT publish_temporary(int directory, const std::string& temporary, const std::string& final,
										bool& published, bool& cleanup_attempted,
		...
		if (rename_errno != ENOSYS && rename_errno != EINVAL && rename_errno != EOPNOTSUPP) {
			return rename_errno == EACCES || rename_errno == EPERM ? LCC_DEVICE_ACCESS_DENIED : LCC_DEVICE_IO_ERROR;
		}
		...
		return link_errno == ENOSYS || link_errno == EINVAL || link_errno == EOPNOTSUPP ? LCC_DEVICE_IO_ERROR
			   : link_errno == EACCES || link_errno == EPERM							? LCC_DEVICE_ACCESS_DENIED
																						: LCC_DEVICE_IO_ERROR;
	}
	...
	LCC_DEVICE_RESULT remove_owned(int directory, const std::string& filename, const FileIdentity& expected,
								   bool& removed, bool require_safe_mode = true) {
		last_rename_unsupported_ = false;
	...
	LCC_DEVICE_RESULT remove_exact_without_rename(int directory, const std::string& filename,
												  const FileIdentity& expected, bool& removed, bool require_safe_mode) {
		removed = false;
		const int raw_descriptor = posix_->openat(directory, filename.c_str(), kReferenceOpenFlags, 0U);
		if (raw_descriptor < 0) {
		...
		if (!valid_removal_status(status, require_safe_mode)) {
			return S_ISREG(status.st_mode) && status.st_uid == ::geteuid() ? LCC_DEVICE_BUSY : LCC_DEVICE_ACCESS_DENIED;
		}
		std::array<std::uint8_t, 16> random{};
		if (openssl_->rand_priv_bytes_ex(libctx_, random.data(), random.size(), kRandomStrength) != 1) {
			return LCC_DEVICE_INTERNAL_ERROR;
		...
		const bool quarantine_matches =
			posix_->fstat(quarantine_descriptor.get(), &quarantine_status) == 0 &&
			valid_removal_status(quarantine_status, require_safe_mode) &&
			same_file(expected, FileIdentity{quarantine_status.st_dev, quarantine_status.st_ino});
...
std::unique_ptr<DeviceKeyProvider> make_tpm2_openssl_provider(std::shared_ptr<OpenSsl3Api> openssl,
															  std::shared_ptr<PosixStorageApi> posix) noexcept {
	try {
		if (!openssl || !posix) {
			return nullptr;
		}
		return std::unique_ptr<DeviceKeyProvider>(new Tpm2OpenSslProvider(std::move(openssl), std::move(posix)));
	} catch (...) {
		return nullptr;
	}
}
...
LCC_DEVICE_RESULT tpm2_openssl_map_error_for_test(int saved_errno, const char* reason, bool loading_reference) {
	try {
		return map_provider_error_text(saved_errno, loading_reference,
...
bool tpm2_openssl_error_queue_segments_for_test() noexcept {
	try {
		...
		const bool isolated =
			text.find("actual-provider-error") != std::string::npos && text.find("benign-probe") == std::string::npos;
		return isolated;
	} catch (...) {
		ERR_clear_error();
		return false;
