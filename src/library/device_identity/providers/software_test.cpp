...
LCC_DEVICE_RESULT export_spki(const std::shared_ptr<SoftwareKey>& key, P256Spki& out) {
    ...
    try {
        ...
        static constexpr std::array<std::uint8_t, 27> prefix = {{
...
LCC_DEVICE_RESULT sign_with_key(const std::shared_ptr<SoftwareKey>& key,
...
public:
    ...
    LCC_DEVICE_RESULT delete_with_expected_id(
        const ProviderOpenRequest& request, const std::string& expected_device_key_id) noexcept override {
        try {
            std::unique_lock<std::timed_mutex> lock(registry_mutex(), std::defer_lock);
            if (!lock_registry(lock, request.lock_timeout_ms)) {
                return LCC_DEVICE_BUSY;
