...
function createCppSourceArchive({ root = repositoryRoot, outputDirectory, consumerId, platformVersion }) {
  const archiveRoot = `licensecc-cpp-sdk-${safeConsumerId(consumerId)}-${safeVersion(platformVersion)}`;
...
function planWorkerAssembly(outputDirectory, root = repositoryRoot) {
  ...
  for (const worker of WORKERS) {
    ...
    if (worker.ui) plan.push({ executable: "npm", args: ["run", "build:ui", "--workspace", worker.workspace, "--", "--outDir", join(work, "ui", worker.name)], cwd: root, label: `${worker.name} isolated UI build` });
...
function runWorkerAssembly({ root = repositoryRoot, outputDirectory, run = commandResult }) {
  ...
  for (const command of planWorkerAssembly(outputDirectory, root)) { const args = command.args.map((arg) => arg === "<local-wrangler-bin>" ? localWrangler : arg); run({ ...command, args }); if (command.outdir && !directoryHasFiles(command.outdir)) throw new Error(`${command.label} produced no bundle files`); }
...
function verifyPackageVersions(records, platformVersion, pythonVersion, requireDotnet = true) {
  ...
  if (requireDotnet && !records.some((record) => record.path.startsWith("dotnet/") && record.path.includes(platformVersion) && record.path.endsWith(".nupkg"))) throw new Error(`NuGet artifact does not carry ${platformVersion}`);
}
function writeReleaseMetadata({ root = repositoryRoot, outputDirectory, platformVersion, pythonVersion, incomplete = false }) {
  const artifacts = payloadRecords(outputDirectory); verifyPackageVersions(artifacts, platformVersion, pythonVersion, !incomplete);
  ...
  const inputs = licenseInputs(root); const packageIds = artifacts.map((_, index) => `SPDXRef-Package-${index + 1}`); const spdx = { spdxVersion: "SPDX-2.3", dataLicense: "CC0-1.0", SPDXID: "SPDXRef-DOCUMENT", name: `licensecc-${platformVersion}`, documentNamespace: `https://github.com/open-license-manager/licensecc/releases/${platformVersion}/spdx`, creationInfo: { created: "1970-01-01T00:00:00Z", creators: ["Tool: licensecc-release-artifacts"] }, documentDescribes: packageIds, packages: artifacts.map((artifact, index) => ({ SPDXID: packageIds[index], name: artifact.path, versionInfo: artifact.path.startsWith("python/") ? pythonVersion : platformVersion, supplier: "NOASSERTION", originator: "NOASSERTION", downloadLocation: "NOASSERTION", filesAnalyzed: false, licenseConcluded: "NOASSERTION", licenseDeclared: "NOASSERTION", copyrightText: "NOASSERTION", checksums: [{ algorithm: "SHA256", checksumValue: artifact.sha256 }] })), files: inputs.map((input, index) => ({ SPDXID: `SPDXRef-License-Input-${index + 1}`, fileName: input.path, checksums: [{ algorithm: "SHA256", checksumValue: input.sha256 }], licenseConcluded: input.licenseConcluded, licenseInfoInFiles: [input.licenseConcluded], copyrightText: "NOASSERTION" })) };
  writeFileSync(join(outputDirectory, "spdx.json"), `${JSON.stringify(spdx, null, 2)}\n`);
  const manifest = { format: "licensecc-release-manifest-v1", platform_version: platformVersion, python_version: pythonVersion, incomplete, artifacts, spdx: "spdx.json" };
...
function inspectReleaseDirectory(outputDirectory, { root = repositoryRoot } = {}) {
  ...
  const manifest = JSON.parse(readFileSync(join(outputDirectory, "release-manifest.json"), "utf8")); if (manifest.format !== "licensecc-release-manifest-v1" || manifest.spdx !== "spdx.json" || !Array.isArray(manifest.artifacts)) throw new Error("unsupported release manifest");
  ...
  const spdx = JSON.parse(readFileSync(join(outputDirectory, "spdx.json"), "utf8")); if (spdx.spdxVersion !== "SPDX-2.3" || !spdx.documentNamespace || !spdx.creationInfo || !Array.isArray(spdx.packages) || !Array.isArray(spdx.files)) throw new Error("invalid SPDX document");
...
function assembleReleaseArtifacts({ root = repositoryRoot, outputDirectory, consumerId, platformVersion, pythonVersion, allowPartial = false, run = commandResult, toolAvailable = (tool) => spawnSync(tool, ["--version"], { stdio: "ignore" }).status === 0 }) {
  const output = resolve(outputDirectory); if (existsSync(output)) throw new Error(`release staging output already exists: ${output}`); safeConsumerId(consumerId); safeVersion(platformVersion); safeVersion(pythonVersion); mkdirSync(output, { recursive: true });
  try {
    ...
    if (hasDotnet) { run({ executable: "dotnet", args: ["restore", join(root, "sdks/dotnet/Licensecc.Client.sln"), "--locked-mode"], cwd: root, label: "locked NuGet restore" }); run({ executable: "dotnet", args: ["pack", join(root, "sdks/dotnet/src/Licensecc.Client/Licensecc.Client.csproj"), "--configuration", "Release", "--no-restore", "--include-symbols", "--include-source", `-p:PackageVersion=${platformVersion}`, "--output", join(output, "dotnet")], cwd: root, label: "NuGet package and symbols" }); }
    createCppSourceArchive({ root, outputDirectory: output, consumerId, platformVersion }); const manifest = writeReleaseMetadata({ root, outputDirectory: output, platformVersion, pythonVersion, incomplete: !hasDotnet }); return inspectReleaseDirectory(output, { root });
  } catch (error) { rmSync(output, { recursive: true, force: true }); throw error; }
}
function parseArgs(argv) { const values = {}; for (let i = 2; i < argv.length; i += 1) { const arg = argv[i]; if (arg === "--allow-partial") { values.allowPartial = true; continue; } if (arg === "--help") return { help: true }; if (!arg.startsWith("--") || argv[i + 1] === undefined) throw new Error(`invalid argument: ${arg}`); values[arg.slice(2)] = argv[++i]; } return values; }
function main() { const options = parseArgs(process.argv); if (options.help) return console.log("usage: node scripts/assemble-release-artifacts.mjs --output <stage> --consumer-id <consumer> --platform-version <semver> --python-version <pep440> [--allow-partial]"); if (!options.output || !options["consumer-id"] || !options["platform-version"] || !options["python-version"]) throw new Error("--output, --consumer-id, --platform-version, and --python-version are required"); console.log(JSON.stringify(assembleReleaseArtifacts({ outputDirectory: options.output, consumerId: options["consumer-id"], platformVersion: options["platform-version"], pythonVersion: options["python-version"], allowPartial: options.allowPartial }), null, 2)); }

export { assertReleaseAllowlist, assertSafeRelativePath, assembleReleaseArtifacts, createCppSourceArchive, inspectReleaseDirectory, planWorkerAssembly, safeConsumerId, trackedCppFiles, writeReleaseMetadata };
