...
function verifyPackageVersions(records, platformVersion, pythonVersion, requireDotnet = true) {
  ...
  if (requireDotnet && (!records.some((record) => record.path.startsWith("dotnet/") && record.path.includes(platformVersion) && record.path.endsWith(".nupkg") && !record.path.endsWith(".symbols.nupkg")) || !records.some((record) => record.path.startsWith("dotnet/") && (record.path.endsWith(".snupkg") || record.path.endsWith(".symbols.nupkg"))))) throw new Error(`NuGet primary and symbols artifacts must carry ${platformVersion}`);
}
...
function assembleReleaseArtifacts({ root = repositoryRoot, outputDirectory, consumerId, expectedPlatformVersion, expectedPythonVersion, allowPartial = false, run = commandResult, toolAvailable = (tool) => spawnSync(tool, ["--version"], { stdio: "ignore" }).status === 0 }) {
  const output = resolve(outputDirectory); const versions = repositoryVersions(root); if ((expectedPlatformVersion !== undefined && expectedPlatformVersion !== versions.platformVersion) || (expectedPythonVersion !== undefined && expectedPythonVersion !== versions.pythonVersion)) throw new Error("supplied expected version does not match tracked version authority"); if (existsSync(output)) throw new Error(`release staging output already exists: ${output}`); safeConsumerId(consumerId); mkdirSync(output, { recursive: true });
  try {
    ...
    createCppSourceArchive({ root, outputDirectory: output, consumerId, cppVersion: versions.cppVersion, platformVersion: versions.platformVersion }); const manifest = writeReleaseMetadata({ root, outputDirectory: output, consumerId, versions, incomplete: !hasDotnet }); return inspectReleaseDirectory(output, { root });
...
export { assertReleaseAllowlist, assertSafeRelativePath, assembleReleaseArtifacts, createCppSourceArchive, inspectReleaseDirectory, planWorkerAssembly, repositoryVersions, safeConsumerId, trackedCppFiles, verifyArchiveGenerator, writeReleaseMetadata };
