#define BOOST_TEST_MODULE test_generator_v201_canonical_payload

#include <boost/test/unit_test.hpp>
#include <build_properties.h>

#include "../src/base_lib/v201_canonical_payload.hpp"
...
static string read_fixture(const string& name) {
	const string path = string(PROJECT_TEST_SRC_DIR) + "/data/v201/" + name;
	ifstream input(path.c_str(), ios::binary);
	BOOST_REQUIRE_MESSAGE(input.is_open(), "can open self-contained v201 fixture " + name);
	if (input.is_open()) {
...
BOOST_AUTO_TEST_CASE(generator_v201_minimal_payload_matches_runtime_vector) {
...
BOOST_AUTO_TEST_CASE(generator_v201_full_payload_matches_runtime_order) {
...
BOOST_AUTO_TEST_CASE(generator_v201_golden_payload_files_match_runtime_vectors) {
...
BOOST_AUTO_TEST_CASE(generator_v201_payload_rejects_noncanonical_metadata) {
