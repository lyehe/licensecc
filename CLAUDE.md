...
| `doc/` | **User-facing reference site** (Sphinx + Breathe + Doxygen): `conf.py`, `Doxyfile`, and `.rst` sources under `analysis/`, `api/`, `usage/`, `development/`, `other/`. Built by `make docs` / `make documentation`. | tracked (docs) -> output in `doc/_build/` (generated on demand, absent in a clean clone), `doc/_doxygen/` |
...
- **`signed_token/`** — shared signed-token core (added 2026-06-15) extracted from the two modules above: envelope split/build, `verify_payload_signature`, the claim-line appenders, `parse_uint64`, the canonical-field parser, and the templated `TrustedKeyOverride<PublicKey>` test seam. `online_verification` and `config_attestation` both depend on it at source level.

> **Shared signed-token core (dedupe DONE 2026-06-15):** the token plumbing once duplicated between `online_verification/OnlineVerification.cpp` and `config_attestation/ConfigAttestation.cpp` (~120-140 lines: `split_envelope`, `build_*_envelope`, `verify_payload_signature`, `append_claim_line`, `parse_uint64`, the canonical-payload field loop, the trusted-key override) now lives in `src/library/signed_token/` (`SignedToken.{hpp,cpp}`). `parse_uint64` was reconciled to the locale-safe range form and the trusted-key override became a per-type `TrustedKeyOverride<PublicKey>`; public headers and all token/error behavior are unchanged. See `docs/superpowers/plans/2026-06-15-structure-cleanup-shared-token-and-service-rename.md` (Workstream A).
...
| `cloudflare-license-admin/` | Admin Worker + Vite/React admin UI for managing entitlements. Default topology (operations back-office, Slice 0): admin + licensing-backend bind the **same** entitlements D1 (one source of truth; backend owns migrations). The `/api/sync/entitlements` forward path + `SYNC_API_TOKEN`/`PUBLIC_VERIFIER_URL` are retained only for an **optional** two-D1 replica setup, not the source of truth. | `src/worker/index.ts`, `src/ui/`, sync + access-drill scripts, `lint.mjs`. |
...
**Offline config signer (`config-sign.mjs`).** Produces `lcccfg1.<b64-payload>.<b64-sig>` tokens signed with `rsa-pkcs1-sha256` (`RSASSA-PKCS1-v1_5` + `SHA-256`), `key-id` of the form `sha256:<64-hex>`, and a `config-hash` of `sha256:<hex>` over the config bytes. CLI options: `--private-key <pkcs8-pem> --key-id sha256:<64-hex> --fingerprint <64-hex> --config <file> --config-id <id> --config-seq <uint> --expires-at <epoch>` (`--expires-at` is required and must be `> --issued-at`; `0` is rejected — config tokens must carry a finite expiry, matching the C++ verifier) plus optional `--project`, `--feature`, `--device-hash <64-hex>`, `--issued-at <epoch>`. This corroborates the config-attestation feature's claims: same envelope/canonical-payload discipline as the online assertion, consumed in C++ by `config_attestation` (not by the Worker).
