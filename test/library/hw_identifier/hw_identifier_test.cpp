...
/**
 * Golden round-trip: pin the exact wire-format bytes / printed strings the
 * current bit layout produces for every strategy. The string is a persistent
 * license contract, so the named-bit refactor must remain identity-preserving.
 */
BOOST_AUTO_TEST_CASE(golden_wire_format) {
	...
	// The public enum reserves CPU and host-name values, but the reconciled
	// runtime intentionally accepts only strategies that it can generate and
	// verify. Keep the byte-vector coverage above without widening that policy.
	HwIdentifier pc_id;
	BOOST_CHECK_THROW(pc_id.set_identification_strategy(LCC_API_HW_IDENTIFICATION_STRATEGY::STRATEGY_CPU_SIZE),
				  logic_error);
	BOOST_CHECK_THROW(pc_id.set_identification_strategy(LCC_API_HW_IDENTIFICATION_STRATEGY::STRATEGY_HOST_NAME),
				  logic_error);
}
