...
LCC_DEVICE_RESULT tpm2_openssl_map_error_for_test(int saved_errno, const char* reason, bool loading_reference);
...
bool tpm2_openssl_decode_der_signature_for_test(const unsigned char* data, std::size_t size, unsigned char* output,
...
public:
	explicit FakeOpenSsl3Api(bool provide_tpm2 = false, bool layered_tpm2_error = false,
	...
	int provider_unload(OSSL_PROVIDER* provider) noexcept override {
		calls.push_back(provider == reinterpret_cast<OSSL_PROVIDER*>(static_cast<std::uintptr_t>(2U))
						...
						: provider == reinterpret_cast<OSSL_PROVIDER*>(static_cast<std::uintptr_t>(3U))
							? "provider_unload:tpm2"
							: "provider_unload:unknown");
		++provider_unload_count;
		return 1;
	}
	EVP_PKEY_CTX* pkey_ctx_new_from_name(OSSL_LIB_CTX* context, const char* name,
	...
	EVP_PKEY_CTX* pkey_ctx_new_from_pkey(OSSL_LIB_CTX* context, EVP_PKEY*, const char* properties) noexcept override {
		if (!full_state_machine_) {
			return nullptr;
		}
		stage = 2;
	...
	int pkey_keygen_init(EVP_PKEY_CTX*) noexcept override { return full_state_machine_ && !fail_keygen ? 1 : 0; }
	...
	int pkey_generate(EVP_PKEY_CTX*, EVP_PKEY** key) noexcept override {
		...
		if (parent_auth_failure != 0) {
			ERR_raise_data(
				ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "%s",
	...
	int pkey_sign_init(EVP_PKEY_CTX*) noexcept override { return full_state_machine_ && !fail_sign ? 1 : 0; }
	...
	int pkey_sign(EVP_PKEY_CTX*, unsigned char* signature, std::size_t* signature_size, const unsigned char*,
	...
	int pkey_verify(EVP_PKEY_CTX*, const unsigned char*, std::size_t, const unsigned char*,
					std::size_t) noexcept override {
		stage = 13;
	...
	int digest(EVP_MD*, const unsigned char*, std::size_t, unsigned char* output,
	...
	int pkey_get_utf8_string_param(const EVP_PKEY*, const char* name, char* value, std::size_t value_size,
	...
	OSSL_ENCODER_CTX* encoder_new_for_pkey(const EVP_PKEY*, int selection, const char* output_type,
										   const char* output_structure, const char* properties) noexcept override {
		...
		saw_private_encoder =
			saw_private_encoder || (encoder_output_type == "PEM" && encoder_output_structure == "PrivateKeyInfo" &&
									encoder_properties == "provider=tpm2");
		saw_public_encoder =
			saw_public_encoder || (encoder_output_type == "DER" && encoder_output_structure == "SubjectPublicKeyInfo");
	...
	OSSL_STORE_CTX* store_open_ex(const char* uri, OSSL_LIB_CTX* context, const char* properties,
								  const UI_METHOD* ui_method, void*) noexcept override {
	...
	OSSL_STORE_INFO* store_load(OSSL_STORE_CTX*) noexcept override {
		if (!full_state_machine_ || store_loaded_count >= store_item_count || store_error_on_load ||
			...
			if (store_error_on_load) {
				store_error_code = 1;
				ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "%s",
...
public:
	int openat(int, const char*, int, mode_t) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	int close(int) noexcept override { return 0; }
	int fstat(int, struct stat*) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	...
	ssize_t write(int, const void*, std::size_t) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	...
	int unlinkat(int, const char*, int) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	int linkat(int, const char*, int, const char*, int) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	int renameat2_noreplace(int, const char*, const char*) noexcept override {
		errno = ENOSYS;
		return -1;
	}
	...
	int nanosleep(const struct timespec*, struct timespec*) noexcept override {
		errno = ENOSYS;
		return -1;
	}
};
...
public:
	explicit LockReachPosixStorageApi(bool full_state_machine = false) : full_state_machine_(full_state_machine) {}
	...
	int fstat(int descriptor, struct stat* status) noexcept override {
		...
		*status = {};
		...
		const bool wrong_owner = (descriptor_info.kind == Kind::Temporary && temporary_wrong_owner) ||
		...
		status->st_mode = descriptor_info.kind == Kind::Directory  ? S_IFDIR | (directory_bad_mode ? 0750U : 0700U)
						  : descriptor_info.kind == Kind::Ancestor ? S_IFDIR | (ancestor_bad_mode ? 0775U : 0755U)
	...
	int unlinkat(int, const char* path, int) noexcept override {
		...
		if (cleanup_name && (fail_cleanup_unlink ||
	...
	int renameat2_noreplace(int, const char* old_path, const char* new_path) noexcept override {
		...
		if (rename_unavailable && old_name.find(".tmp.") != std::string::npos && new_name.size() >= 9U &&
			new_name.substr(new_name.size() - 9U) == ".tss2.pem") {
			errno = ENOSYS;
			return -1;
		}
		...
		if (force_publish_exists && old_name.find(".tmp.") != std::string::npos && new_name.size() >= 9U &&
			new_name.substr(new_name.size() - 9U) == ".tss2.pem") {
...
ProviderOpenRequest request_for(const std::string& storage) {
	...
	require(license::device_identity::derive_namespace_v1("licensecc.test.tpm2-shim", "DEFAULT", request.scope,
...
void test_provider_load_order_and_unavailable_mapping() {
	...
	auto provider =
		license::device_identity::make_tpm2_openssl_provider(openssl, std::make_shared<NullPosixStorageApi>());
...
void test_provider_load_unknown_and_allocator_failures_remain_internal() {
	...
	provider = license::device_identity::make_tpm2_openssl_provider(openssl, std::make_shared<NullPosixStorageApi>());
	require(provider->open(request_for("/var/lib/licensecc")) == LCC_DEVICE_INTERNAL_ERROR,
...
void test_error_queue_preserved_when_initially_empty() {
	ERR_clear_error();
	auto provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(),
																		 std::make_shared<NullPosixStorageApi>());
...
void test_layered_provider_error_maps_first_cause_and_preserves_caller_queue() {
	...
	ERR_raise(ERR_LIB_USER, ERR_R_INTERNAL_ERROR);
	const unsigned long before = ERR_peek_last_error();
	auto provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(false, true),
																		 std::make_shared<NullPosixStorageApi>());
...
void test_provider_error_mapping_and_private_reference_type() {
	...
	require(tpm2_openssl_map_error_for_test(0, "the TPM was not able to start the command", false) == LCC_DEVICE_BUSY,
	...
	require(tpm2_openssl_map_error_for_test(0, "cannot connect to TPM", false) == LCC_DEVICE_HARDWARE_UNAVAILABLE,
			"cannot connect must map to hardware unavailable");
	require(
		tpm2_openssl_map_error_for_test(0, "cannot get capability from TPM", false) == LCC_DEVICE_HARDWARE_UNAVAILABLE,
		"capability failure must map to hardware unavailable");
	require(tpm2_openssl_map_error_for_test(
				0, "cannot create primary 2466 tpm:session(1):authorization failure without DA implications", false) ==
	...
	require(tpm2_openssl_map_error_for_test(0, "transport unavailable", false) == LCC_DEVICE_HARDWARE_UNAVAILABLE,
			"transport loss must map to hardware unavailable");
	require(tpm2_openssl_map_error_for_test(0, "unsupported algorithm", false) == LCC_DEVICE_UNSUPPORTED_ALGORITHM,
	...
	require(tpm2_openssl_accepts_tss2_private_pem_for_test(reinterpret_cast<const unsigned char*>(valid.data()),
														   valid.size()),
			"valid TSS2 PEM rejected");
	require(!tpm2_openssl_accepts_tss2_private_pem_for_test(reinterpret_cast<const unsigned char*>(ordinary.data()),
															ordinary.size()),
			"ordinary private-key PEM accepted");
	require(!tpm2_openssl_accepts_tss2_private_pem_for_test(reinterpret_cast<const unsigned char*>(trailing.data()),
															trailing.size()),
...
void test_der_signature_edges_are_rejected_before_result_mapping() {
	...
	require(tpm2_openssl_accepts_der_signature_for_test(valid.data(), valid.size()), "valid DER signature rejected");
	...
	const auto encode_integer = [](const std::array<unsigned char, 32>& scalar, std::vector<unsigned char>& body) {
		std::size_t first = 0U;
	...
	const auto make_der = [&](const std::array<unsigned char, 32>& r, const std::array<unsigned char, 32>& s) {
	...
	const std::array<unsigned char, 32> order = {{0xffU, 0xffU, 0xffU, 0xffU, 0x00U, 0x00U, 0x00U, 0x00U,
	...
	require(tpm2_openssl_decode_der_signature_for_test(high_s.data(), high_s.size(), decoded_high_s.data(),
													   decoded_high_s.size()),
			"valid high-S DER signature rejected");
	require(
		std::equal(decoded_high_s.begin(), decoded_high_s.begin() + 31U, std::array<unsigned char, 31U>{}.begin()) &&
...
void test_create_reaches_namespace_lock_after_directory_open() {
	...
	require((storage->selected_directory_flags & O_NOFOLLOW) != 0, "selected storage directory must reject symlinks");
...
void test_nonregular_reference_rejects_without_blocking_open() {
	...
	require((storage->reference_flags & O_NONBLOCK) != 0, "reference open must use O_NONBLOCK before type validation");
	...
	require((storage->lock_flags & O_NONBLOCK) != 0, "lock open did not use O_NONBLOCK before type validation");
}
...
void test_storage_owner_mode_and_symlink_matrix() {
	...
	auto provider =
		license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_ACCESS_DENIED, "unsafe lock mode was accepted");
	storage = std::make_shared<LockReachPosixStorageApi>();
	storage->lock_wrong_owner = true;
	provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_ACCESS_DENIED, "unsafe lock ownership was accepted");
}
...
void test_parent_auth_and_prepublication_failures_leave_no_reference() {
	...
	const auto expect_failure_without_publish = [](bool digest_failure, bool sign_failure, bool verify_failure,
												   bool malformed_der, bool encoder_failure) {
		auto openssl = std::make_shared<FakeOpenSsl3Api>(true, false, true);
...
void test_store_terminal_and_provider_failure_matrix() {
	...
	auto terminal_provider =
		license::device_identity::make_tpm2_openssl_provider(terminal_openssl_error, terminal_storage);
	...
	require(dirty_result == LCC_DEVICE_KEY_CORRUPT, "terminal STORE probe with a provider error queue was accepted");
...
void test_storage_ancestor_symlink_lock_timeout_and_publish_capabilities() {
	...
	provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/ancestor/safe")) == LCC_DEVICE_ACCESS_DENIED, "ancestor symlink was accepted");
	...
	provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_IO_ERROR, "lock symlink did not fail closed");

	storage = std::make_shared<LockReachPosixStorageApi>();
	storage->lock_busy = true;
	provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_BUSY, "lock timeout was not busy");

	storage = std::make_shared<LockReachPosixStorageApi>();
	storage->lock_busy = true;
	storage->lock_releases_after_deadline = true;
	provider = license::device_identity::make_tpm2_openssl_provider(std::make_shared<FakeOpenSsl3Api>(true), storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_BUSY, "lock became available after the deadline");
	...
	require(provider->create(request_for("/safe")) == LCC_DEVICE_OK, "EINVAL no-replace fallback did not publish");
	...
	provider = license::device_identity::make_tpm2_openssl_provider(
		std::make_shared<FakeOpenSsl3Api>(true, false, true), storage);
	require(provider->create(request_for("/safe")) == LCC_DEVICE_OK, "rename no-replace winner was not adopted");
	require(storage->reference_present && !storage->temporary_present, "rename winner adoption left wrong entries");
	...
	require(exact_delete_result == LCC_DEVICE_OK, "expected-id delete did not use the exact no-rename fallback");
	...
	require(storage->temporary_present, "fallback revalidation race removed the replacement temporary entry");
...
void test_publish_cleanup_preserves_a_same_name_replacement() {
	...
	auto provider = license::device_identity::make_tpm2_openssl_provider(openssl, storage);
	const LCC_DEVICE_RESULT result = provider->create(request_for("/safe"));
	require(result == LCC_DEVICE_BUSY, "publish cleanup race was not reported as busy");
...
void test_store_cardinality_and_clean_eof_are_required() {
	...
	storage->reference_present = true;
	provider = license::device_identity::make_tpm2_openssl_provider(openssl, storage);
	require(provider->open(request_for("/safe")) == LCC_DEVICE_KEY_CORRUPT, "STORE without a clean EOF was accepted");
...
void test_no_replace_fallback_winner_and_publish_rollback() {
	...
	storage->rename_unavailable = true;
	auto provider = license::device_identity::make_tpm2_openssl_provider(openssl, storage);
	require(provider->create(request_for("/safe")) == LCC_DEVICE_OK, "hard-link no-replace fallback did not publish");
	require(storage->reference_present && !storage->temporary_present, "hard-link fallback left a temporary reference");
	...
	provider = license::device_identity::make_tpm2_openssl_provider(openssl, storage);
	require(provider->create(request_for("/safe")) == LCC_DEVICE_OK, "hard-link EEXIST winner was not adopted");
	require(storage->reference_present && !storage->temporary_present,
	...
	require(fsync_result == LCC_DEVICE_IO_ERROR, "publish fsync failure was reported as success");
	require(!storage->reference_present && !storage->temporary_present,
...
void test_temporary_owner_and_mode_validation_is_exercised() {
	...
	require(temporary_mode_result == LCC_DEVICE_ACCESS_DENIED, "temporary mode validation was not exercised");
	...
	require(temporary_owner_result == LCC_DEVICE_ACCESS_DENIED, "temporary owner validation was not exercised");
}
...
void test_shim_successful_open_sign_close_1000_cycles() {
	...
	for (unsigned int cycle = 0U; cycle < 1000U; ++cycle) {
		...
		require(provider->sign_digest(digest, signature) == LCC_DEVICE_OK, "successful shim sign cycle failed");
		provider.reset();
		require(openssl->provider_unload_count == 2 && openssl->libctx_free_count == 1 && openssl->pkey_free_count >= 2,
				"successful shim cycle leaked provider context or key handles");
		require(openssl->calls.size() >= 3U && openssl->calls[openssl->calls.size() - 3U] == "provider_unload:tpm2" &&
...
int run_real(const char* storage_directory) {
	...
	if (hardware_opt_in) {
		...
		if (tcti == nullptr || *tcti == '\0' || storage_directory == nullptr ||
			...
			std::cerr << "real TPM2 tests require a caller-supplied TPM2OPENSSL_TCTI and an existing 0700 storage "
						 "directory\n";
			return 2;
		}
	}
...
constexpr std::array<std::uint8_t, 32> kDigest = {{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa,
												   0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60,
												   0x70, 0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0xe0, 0xf0, 0x01}};

constexpr std::array<std::uint8_t, 32> kP256Order = {{0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff,
...
bool read_der_integer(const std::uint8_t* der, std::size_t size, std::size_t& offset, const std::uint8_t*& value,
...
std::string provider_param(const OSSL_PROVIDER* provider, const char* name) {
	char* value = nullptr;
	OSSL_PARAM params[] = {OSSL_PARAM_construct_utf8_ptr(name, &value, 0U), OSSL_PARAM_construct_end()};
...
bool export_public_spki(OSSL_LIB_CTX* libctx, EVP_PKEY* key, Pkey& out) {
	EncoderContext encoder(
		OSSL_ENCODER_CTX_new_for_pkey(key, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", nullptr),
...
bool sign_digest(OSSL_LIB_CTX* libctx, EVP_PKEY* key, const std::array<std::uint8_t, 32>& digest,
	...
	if (!context || EVP_PKEY_sign_init(context.get()) <= 0 || !sha256 ||
		EVP_PKEY_CTX_set_signature_md(context.get(), sha256.get()) <= 0) {
...
bool verify_digest(OSSL_LIB_CTX* libctx, EVP_PKEY* public_key, const std::vector<std::uint8_t>& signature,
				   const std::array<std::uint8_t, 32>& digest, int expected) {
	PkeyContext context(EVP_PKEY_CTX_new_from_pkey(libctx, public_key, "provider=default"), EVP_PKEY_CTX_free);
	MessageDigest sha256(EVP_MD_fetch(libctx, "SHA256", "provider=default"), EVP_MD_free);
	if (!context || EVP_PKEY_verify_init(context.get()) <= 0 || !sha256 ||
		EVP_PKEY_CTX_set_signature_md(context.get(), sha256.get()) <= 0) {
	...
	return require(result == expected,
				   expected == 1 ? "original digest verification failed" : "double-hashed digest was accepted");
}
...
bool run_capability() {
	...
	OSSL_PARAM generation_params[] = {OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, group, 0U),
									  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_DIGEST, digest_name, 0U),
									  OSSL_PARAM_construct_uint("parent", &parent), OSSL_PARAM_construct_end()};
	...
	if (!require(owning_provider != nullptr && std::string(OSSL_PROVIDER_get0_name(owning_provider)) == "tpm2",
	...
	if (!require(EVP_PKEY_get_utf8_string_param(generated.get(), OSSL_PKEY_PARAM_GROUP_NAME, actual_group.data(),
												actual_group.size(), &actual_group_size) > 0 &&
	...
	std::cout << "signature_der_bytes=" << signature_der.size() << " signature_p1363_bytes=" << signature_p1363.size()
			  << '\n';
	...
	if (!require(EVP_Q_digest(libctx.get(), "SHA256", "provider=default", kDigest.data(), kDigest.size(),
							  double_hash.data(), &double_hash_size) == 1 &&
