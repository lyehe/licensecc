...
BOOST_AUTO_TEST_CASE(expected_id_deletion_is_guarded_and_close_is_non_destructive) {
	...
	std::fill(options.project + std::strlen(options.project) + 1U, options.project + sizeof(options.project), 'y');
	BOOST_REQUIRE(lcc_device_identity_open(&options, &handle) == LCC_DEVICE_OK);
	...
	BOOST_TEST(lcc_device_identity_delete_key(&options, exact_id_with_ignored_trailing_byte.data()) == LCC_DEVICE_OK);
...
BOOST_AUTO_TEST_CASE(expected_id_nontermination_is_bounded_at_max_plus_one) {
	...
	BOOST_REQUIRE(VirtualProtect(static_cast<unsigned char*>(allocation) + page_size, page_size, PAGE_NOACCESS,
								 &old_protection) != 0);
	...
	void* allocation = ::mmap(nullptr, page_size * 2U, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
