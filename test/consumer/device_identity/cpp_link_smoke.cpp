...
int main() {
	...
	if (open_result == LCC_DEVICE_OK) {
		...
		return metadata_result == LCC_DEVICE_OK && metadata.backend == LCC_DEVICE_BACKEND_WINDOWS_TPM &&
	...
	if (metadata_result != LCC_DEVICE_OK || metadata.backend != LCC_DEVICE_BACKEND_TPM2_OPENSSL ||
		...
		std::string(metadata.provider) != "tpm2-openssl" || std::string(metadata.algorithm) != "ecdsa-p256-sha256") {
		return 3;
	}
	options.flags = 0U;
