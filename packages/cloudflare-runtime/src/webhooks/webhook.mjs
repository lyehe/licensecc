...
/**
 * Web Crypto deliberately accepts only ArrayBuffer-backed views. A secret map can
 * legitimately hold a view over a SharedArrayBuffer, so make a short-lived private
 * copy before importing a key rather than relying on a cast or a Worker-specific
 * buffer implementation.
 *
 * @param {Uint8Array} bytes
 * @returns {ArrayBuffer}
 */
function copyForWebCrypto(bytes) {
