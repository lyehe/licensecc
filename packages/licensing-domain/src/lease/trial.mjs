// Portable server-computed trial timing decision.
//
 ...
 *       allow. `stamp` tells the D1 adapter to write trial_started_at=now + trial_device_hash=lockKey
 *       WRITE-ONCE in the lease batch. `trialExpiresAt` is the clamp deadline (trial_started_at + trial_duration_sec)
