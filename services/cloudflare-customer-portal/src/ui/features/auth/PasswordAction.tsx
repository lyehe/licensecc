...
export function PasswordAction({ token, onDone }: { token: string; onDone(): Promise<void> }): React.ReactElement {
  ...
  async function submit(event: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      if (result.ok) { setFinished(true); await onDone(); }
  ...
  return <main className="authPane"><section className="authCard">
    ...
    {token && !finished && <form onSubmit={event => void submit(event)}>
      <label>New password<input type="password" autoComplete="new-password" required minLength={15} maxLength={256} value={password} onChange={event => setPassword(event.target.value)} /></label>
      <label>Confirm password<input type="password" autoComplete="new-password" required maxLength={256} value={confirmation} onChange={event => setConfirmation(event.target.value)} /></label>
