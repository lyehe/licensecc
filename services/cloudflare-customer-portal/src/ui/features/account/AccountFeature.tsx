...
export function AccountFeature({ customerId, busy, logout }: {
  customerId: string | null; busy: boolean; logout(): Promise<void>;
}): React.ReactElement {
  ...
  return <div className="accountPage">
    ...
    {(failed || identityError || !providers || !identities || providers.google || providers.github || identities.length > 0) && <section className="tablePane full"><h2>Connected accounts</h2>
      <p>Use a connected account to sign in.</p>
    ...
    <section className="tablePane full"><h2>Sign out</h2><p>Your apps and devices stay connected.</p><button disabled={busy} onClick={() => void logout()}>Sign out</button></section>
