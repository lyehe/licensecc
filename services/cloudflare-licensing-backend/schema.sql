...
CREATE TABLE account_token_revocations (
...
CREATE TABLE entitlements (
  ...
  revocation_seq INTEGER NOT NULL DEFAULT 0,
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL, valid_from INTEGER NULL, valid_until INTEGER NULL, notes TEXT NOT NULL DEFAULT '', customer_id TEXT NULL, license_id TEXT NULL, max_active_devices INTEGER NOT NULL DEFAULT 1, lease_seconds INTEGER NOT NULL DEFAULT 2592000, rebind_window_sec INTEGER NOT NULL DEFAULT 7776000, pool_size INTEGER NOT NULL DEFAULT 0, heartbeat_grace_sec INTEGER NOT NULL DEFAULT 900, max_borrow_sec INTEGER NOT NULL DEFAULT 0, allow_overdraft INTEGER NOT NULL DEFAULT 0, last_applied_order_seq INTEGER NOT NULL DEFAULT 0, last_applied_order_epoch INTEGER NOT NULL DEFAULT 0, policy_id TEXT NULL, is_trial INTEGER NOT NULL DEFAULT 0, trial_expiration_basis TEXT NULL, trial_duration_sec INTEGER NOT NULL DEFAULT 0, trial_one_per_device INTEGER NOT NULL DEFAULT 0, trial_require_device_proof INTEGER NOT NULL DEFAULT 0, trial_started_at INTEGER NULL, trial_device_hash TEXT NULL, meter_quota INTEGER NOT NULL DEFAULT 0, meter_period_sec INTEGER NOT NULL DEFAULT 2592000,
  PRIMARY KEY (project, feature, license_fingerprint)
);

CREATE TABLE lease_issuance (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  project TEXT NOT NULL,
  feature TEXT NOT NULL,
  license_fingerprint TEXT NOT NULL,
...
CREATE TABLE order_ingest_nonces (
  key_id      TEXT NOT NULL,
  event_id    TEXT NOT NULL,
...
CREATE TABLE portal_bootstrap_events (
  id          TEXT PRIMARY KEY,
  customer_id TEXT NOT NULL,
...
CREATE TABLE request_proof_nonces (
  project TEXT NOT NULL,
  feature TEXT NOT NULL,
  license_fingerprint TEXT NOT NULL,
  device_key_id TEXT NOT NULL,
...
CREATE INDEX idx_account_token_events_customer ON account_token_events(customer_id);

CREATE INDEX idx_account_token_events_token ON account_token_events(account_token_id);

CREATE INDEX idx_account_tokens_customer ON account_tokens(customer_id);

CREATE UNIQUE INDEX idx_account_tokens_hmac ON account_tokens(token_hmac);

CREATE INDEX idx_account_tokens_status ON account_tokens(status);

CREATE INDEX idx_audit_digests_source ON audit_digests(source, id);

CREATE INDEX idx_catalog_events_entity
  ON catalog_events(entity_type, entity_id, created_at DESC);

CREATE INDEX idx_catalog_events_project
  ON catalog_events(project, created_at DESC);

CREATE INDEX idx_catalog_features_project_status
  ON catalog_features(project, status);

CREATE UNIQUE INDEX idx_catalog_plan_features_addon
  ON catalog_plan_features(plan_id, addon_key)
  WHERE addon_key IS NOT NULL;

CREATE INDEX idx_catalog_plan_features_project
  ON catalog_plan_features(project, plan_id, status);

CREATE INDEX idx_catalog_plans_project_status
  ON catalog_plans(project, status);

CREATE INDEX idx_customer_events_customer ON customer_events(customer_id, created_at DESC);

CREATE UNIQUE INDEX idx_customers_email
  ON customers(lower(email))
  WHERE email <> '';

CREATE INDEX idx_entitlement_devices_entitlement
  ON entitlement_devices(project, feature, license_fingerprint);

CREATE INDEX idx_entitlement_devices_status
  ON entitlement_devices(status);

CREATE INDEX idx_entitlement_events_actor
  ON entitlement_events(actor, created_at);

CREATE INDEX idx_entitlement_events_lookup
  ON entitlement_events(project, feature, license_fingerprint, created_at);

CREATE INDEX idx_entitlement_events_request
  ON entitlement_events(request_id);

CREATE UNIQUE INDEX idx_entitlement_policies_name ON entitlement_policies(project, lower(name));

CREATE INDEX idx_entitlements_customer
  ON entitlements(customer_id);

CREATE INDEX idx_entitlements_license
  ON entitlements(license_id);

CREATE INDEX idx_entitlements_project_feature_status
  ON entitlements(project, feature, status);

CREATE INDEX idx_entitlements_status
  ON entitlements(status);

CREATE INDEX idx_entitlements_valid_until
  ON entitlements(valid_until);

CREATE INDEX idx_lease_issuance_entitlement
  ON lease_issuance(project, feature, license_fingerprint, issued_at);

CREATE INDEX idx_lease_issuance_issued_at
  ON lease_issuance(issued_at);

CREATE INDEX idx_license_plan_assignments_customer
  ON license_plan_assignments(customer_id);

CREATE INDEX idx_license_plan_assignments_plan
  ON license_plan_assignments(project, plan_id, status);

CREATE INDEX idx_licenses_customer
  ON licenses(customer_id);

CREATE INDEX idx_licenses_project
  ON licenses(project);

CREATE INDEX idx_mutation_idempotency_created_at
  ON mutation_idempotency(created_at);

CREATE INDEX idx_order_events_sub_seq
...
CREATE INDEX idx_order_ingest_nonces_expires_at
  ON order_ingest_nonces(expires_at);

CREATE UNIQUE INDEX idx_orders_fp_unique
  ON orders(project, feature, license_fingerprint);

CREATE INDEX idx_policy_events_policy ON policy_events(policy_id, created_at DESC);

CREATE INDEX idx_portal_bootstrap_customer ON portal_bootstrap_events(customer_id);

CREATE UNIQUE INDEX idx_portal_otp_code ON portal_otp(code_hmac);

CREATE INDEX idx_portal_otp_expires ON portal_otp(expires_at);

CREATE UNIQUE INDEX idx_portal_otp_secret ON portal_otp(secret_hmac);
...
CREATE UNIQUE INDEX idx_portal_sessions_hmac ON portal_sessions(session_hmac);

CREATE INDEX idx_rate_limit_counters_expires_at
  ON rate_limit_counters(expires_at);

CREATE INDEX idx_request_proof_nonces_expires_at
  ON request_proof_nonces(expires_at);

CREATE INDEX idx_seat_checkouts_live
  ON seat_checkouts(project, feature, license_fingerprint, heartbeat_deadline);

CREATE INDEX idx_usage_events_ts
  ON usage_events(ts);

CREATE INDEX idx_usage_events_window
  ON usage_events(project, feature, license_fingerprint, ts);

CREATE INDEX idx_usage_meters_entitlement
