...
// switches never disable this gate. All v2 operations share the same budgets.
export async function limitBoundRequest(request, env, db) {
  ...
  const result = await db.batch([
    ...
    db.prepare(insert + values + admission + conflict).bind("device-v2-client", client),
  ]);
  ...
  if (individual > 20) throw new BoundRequestError("rate_limited", 429);
  if (global === 1) {
    await db.prepare(`DELETE FROM rate_limit_counters WHERE rowid IN
      (SELECT rowid FROM rate_limit_counters WHERE namespace IN ('device-v2-global','device-v2-client')
        AND expires_at<=unixepoch() ORDER BY expires_at LIMIT 2002)`).run();
  }
}
